...
# 每章必需的小节标题（中文模式 + 英文模式）
# 根据HeavySkill审查要求：增强格式解析鲁棒性，识别更多合法格式变体
# 但保持≥60分阈值及必备小节不可缺
_REQUIRED_SECTIONS = [
    {
        "name": "结论要点",
...
def structural_check(
    ...
    # ---- 检查 1.5: 章节固化——H1 唯一性（防 LLM 自造 # 第N章 标题 / 模板泄漏） ----
    # 章节正文不应含 H1；若 LLM 自造 "# 第X章" 或 "# 标题" → 严重违规
    h1_matches = re.findall(r"(?m)^#\s+.*$", content)
    h1_count = len(h1_matches)
    self_made_chapter = [m.strip() for m in h1_matches
                         if re.search(r"#\s*第\s*\d+\s*[章回节]", m)]
    details["checks"]["h1_count"] = h1_count
    details["checks"]["self_made_chapters"] = self_made_chapter

    if h1_count > 0:
        if self_made_chapter:
            issues.append(
                f"[critical] 章节 {chapter_id} 含自造的章节 H1 标题: "
                f"{', '.join(self_made_chapter[:3])}（必须删除，章节标题由组装层统一添加）"
            )
        else:
            issues.append(
                f"[major] 章节 {chapter_id} 含 {h1_count} 个 H1 标题（应使用 H2/H3，"
                f"H1 保留给组装层章节标题）"
            )
    ...
    if placeholders_found:
        issues.append(
            ...
            + ", ".join(sorted(set(placeholders_found))[:5])
        )
    ...
    if contract and contract.get("must_answer"):
        ...
        for item in must_answer_items:
            ...
            if keywords and keywords.lower() in content.lower():
        ...
        if unanswered_items:
            ...
            issues.append(
                f"[{severity}] must_answer未全部回答: "
                f"{len(unanswered_items)}/{len(must_answer_items)}项未覆盖"
            )
