...
class Validators:
    ...
    @staticmethod
    def cross_validate_pe_pb(
        pe_value: float,
        pb_value: float,
        eps: float,
        bvps: float,
        current_price: float,
        tolerance_pe: float = 0.10,
        tolerance_pb: float = 0.10,
    ) -> ValidationResult:
        """交叉验证PE和PB估值的一致性

        同方法内验证：PE×EPS vs PB×BPS vs 当前股价
        ...
        Args:
            ...
            bvps: 每股净资产
            current_price: 当前股价
            tolerance_pe: PE估值偏差容忍度（默认10%）
            tolerance_pb: PB估值偏差容忍度（默认10%）
        ...
        # 计算PE隐含估值
        pe_implied = pe_value * eps if eps > 0 else 0

        # 计算PB隐含估值
        pb_implied = pb_value * bvps if bvps > 0 else 0

        # 检查1: PE隐含估值 vs 当前股价
        if current_price > 0 and pe_implied > 0:
            pe_deviation = abs(pe_implied - current_price) / current_price
            checks.append(CheckResult(
                name="PE隐含估值 vs 当前股价",
                passed=pe_deviation <= tolerance_pe,
                expected=f"{current_price:.2f}",
                actual=f"{pe_implied:.2f}",
                message=f"PE隐含估值偏差: {pe_deviation:.2%} (允许: {tolerance_pe:.2%})",
            ))

        # 检查2: PB隐含估值 vs 当前股价
        if current_price > 0 and pb_implied > 0:
            pb_deviation = abs(pb_implied - current_price) / current_price
            checks.append(CheckResult(
                name="PB隐含估值 vs 当前股价",
                passed=pb_deviation <= tolerance_pb,
                expected=f"{current_price:.2f}",
                actual=f"{pb_implied:.2f}",
                message=f"PB隐含估值偏差: {pb_deviation:.2%} (允许: {tolerance_pb:.2%})",
            ))

        # 检查3: PE隐含估值 vs PB隐含估值一致性
        if pe_implied > 0 and pb_implied > 0:
            cross_deviation = abs(pe_implied - pb_implied) / min(pe_implied, pb_implied)
            checks.append(CheckResult(
                name="PE与PB估值一致性",
                passed=cross_deviation <= 0.20,
                expected=f"PE:{pe_implied:.2f} PB:{pb_implied:.2f}",
                actual=f"偏差: {cross_deviation:.2%}",
                message=f"PE与PB估值偏差: {cross_deviation:.2%} (允许: 20%)",
            ))
