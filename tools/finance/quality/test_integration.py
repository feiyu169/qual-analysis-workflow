...
# HGF P0-①（2026-08-22）：本文件是 hermes 搬运的脚本式集成测试——函数参数
# （reasoning_result/report）由 main() 链式传入，pytest 会误判为 fixture。
# 显式 skip 链式函数（三态：⚠️ 需 main() 驱动），保留独立函数（test_causal_inference_chain/test_cold_start）。
...
@pytest.mark.skip(reason="脚本式链调用（main() 串联），pytest 下参数被误判为 fixture——需 main() 驱动")
