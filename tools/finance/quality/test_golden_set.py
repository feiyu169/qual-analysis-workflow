...
def test_wanhua_golden_set():
    ...
    # HGF P0-①：补断言（原为 print 式演示，无断言）
    assert pe_result.value > 0, "PE 应为正"
    assert pb_result.value > 0, "PB 应为正"
    assert mc_result.value > 0, "市值应为正"
    assert pb_validation.is_valid, f"PB 校验应通过: {pb_validation.warnings}"
...
def test_kuaishou_golden_set():
    ...
    # HGF P0-①：补断言
    assert pe_gaap.value > 0, "PE(GAAP) 应为正"
    assert pe_adjusted.value > 0, "PE(Adjusted) 应为正"
    assert mc_result.value > 0, "市值应为正"
    assert pe_validation.is_valid, f"PE 校验应通过: {pe_validation.warnings}"
...
def test_reasoning_chain():
    ...
    # HGF P0-①：补断言
    assert result is not None, "推理结果不应为 None"
    assert result.causal_graph is not None, "因果图不应为 None"
    ...
    reasoning_result = ReasoningResult(
        ...
        counter_result=CounterResult(
            ...
            falsification_indicators=[
                FalsificationIndicator(name="ind1", description="Test",
