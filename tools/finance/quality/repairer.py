...
## 输出要求
只输出修改点（patch），不要输出整个章节。格式为 JSON：
...
2. **只修复审计点名的位置**，未点名内容一个字节都不要动
3. **最多 5 个 patch**；超出则本轮修复失败
4. 只输出 JSON，不要其他文字
"""
        ...
        if needs_repair:
            ...
            if llm_caller is None:
                ...
                if repaired and len(repaired.strip()) > 200:
                    ...
                    repaired_len = len(repaired.strip()) if repaired else 0
                    logger.warning(
                        f"修复 {chapter_id} 第 {round_num} 轮: "
                        f"LLM 返回内容过短({repaired_len}字符<200)，保留原内容"
                    )
...
def _call_llm_repair(
    ...
    try:
        ...
        if patches:
            # 校验器：结构（H1 唯一性）+ 数字（无 Wind 时跳过）
            def _structural(c: str) -> list:
                r = structural_check(chapter_id, c, contract)
                return r.issues if not r.passed else []

            result = apply_patches(content, patches, validators=[_structural])
            if result.ok and result.applied:
                logger.info(f"patch 修复成功: {chapter_id} {len(result.applied)} 处")
                return result.content
            elif result.rollback:
                logger.warning(f"patch 校验失败回滚: {chapter_id}")
                return None

        # 降级：原整章输出（仅当 patch 解析失败）
