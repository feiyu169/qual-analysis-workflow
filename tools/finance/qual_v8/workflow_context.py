...
class WorkflowContext:
    ...
    def __init__(self, config: QualConfig | None = None):
    ...
    def on_step_start(self, step_name: str, step_num: int | None = None):
        """Step开始时的钩子（非侵入式）"""
    ...
    def on_step_end(self, step_name: str, step_num: int | None = None,
                    passed: bool = True, details: dict | None = None):
        """Step结束时的钩子（非侵入式）"""
        if not self._initialized:
            return
    ...
    def on_error(self, step_name: str, error: Exception, step_num: int | None = None):
        """错误发生时的钩子（非侵入式）"""
        if not self._initialized:
            return
    ...
    def _build_execution_log(self, step_name: str, step_num: int,
                             passed: bool, details: dict | None) -> dict[str, Any]:
        """构建执行日志（用于第三方监督）"""
        return {
            f"gate_{step_num}": {
                "passed": passed,
...
def get_workflow_context(config: QualConfig | None = None) -> WorkflowContext:
    """获取工作流上下文（单例）"""
