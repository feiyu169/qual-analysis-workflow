...
from finance.downloaders.hkexnews_downloader import HKEXNewsDownloader
from finance.downloaders.http_client import HttpClient

# 导入被测模块
from finance.downloaders.models import (
    contains_cjk,
...
from finance.downloaders.sec_downloader import SECDownloader
...
# HGF P0-①（2026-08-22）：以下 3 个类测试的是 hermes 版下载器 API
# （http_client 构造注入 + _infer_fiscal_year/_is_amended_title/_split_stock_code_tokens
#  /_announcement_matches_stock/_infer_period/_infer_year/_resolve_exchange_context
#  /_is_title_blocked 等方法），本项目下载器实现契约不同（_make_request + 本地方法）——
# 对应实现未随迁，显式 skip 标注（三态：⚠️ 已知不兼容），不静默放行。
_HERMES_DL_API_NOT_MIGRATED = (
    "hermes 版下载器 API 未随迁（http_client 注入 + 辅助方法），"
    "本项目下载器契约不同——见 docs/qual-hgf-deep-check-2026-08-22.md §3"
)


@pytest.mark.skip(reason=_HERMES_DL_API_NOT_MIGRATED)
