# HGF + HeavySkill 结合技术文档（V8.0）

> 基于 V7 审查意见修复
> 日期：2026-06-21
> 修复：P0 正则语法/industry 统一、P1 大小写/升级条件/降级阈值/coverage 类型、P2 Token/降级/warnings
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| V1.0-V6.0 | 2026-06-21 | 历史版本 |
| V7.0 | 2026-06-21 | 修复 snake_case 匹配、asyncio 逻辑、upgrade_conditions、process 处理 |
| V8.0 | 2026-06-21 | 修复正则语法、industry 统一、大小写、升级条件匹配、降级阈值、coverage 类型 |
...
# V8 修复：正则表达式语法正确，- 放在末尾
_TOKENIZER = re.compile(r'[_.\\/\-]')  # 匹配 _ . \ / -
...
class RiskAssessor:
    ...
    def _get_file_sensitivity(self, area: str) -> int:
        ...
        for keyword, score in self.FILE_SENSITIVITY.items():
            if keyword in parts:  # 精确 token 匹配
                return score
        return 3
    ...
    def assess(self, affected_areas: List[str], description: str = "") -> RiskAssessment:
        ...
        for area in affected_areas:
            risk_score += self._get_file_sensitivity(area)
...
class ConclusionValidatorInterface:
    def validate(
        ...
        checklist: Checklist,
        config: Dict[str, Any],
        code_context: str = ""  # V8 新增：代码内容，用于升级条件匹配
    ) -> ValidationResult:
        pass
```
...
- id: "S-01"
  ...
  auto_downgrade:  # V8 新增
    enabled: true
    threshold: 3   # 3 次误报就降级
    min_severity: P1  # 最低降到 P1
```
...
**V8 修复：统一 industry 名称，不带连字符**

```yaml
project:
  industries: [healthcare, finance]  # V8 修复：统一不带连字符
...
checklists:
  severity_overrides:
    - industry: finance
      overrides:
        D-07: P0
    - industry: healthcare  # V8 修复：与 project.industries 一致
      overrides:
        S-11: P0
```
    ...
    def apply_severity_overrides(self, checklist: Checklist, project_config: Dict) -> Checklist:
        ...
        for industry in industries:
            for override in self.config.get("severity_overrides", []):
                if override["industry"] == industry:  # V8：名称已统一
...
class ChecklistManager:
    ...
    def __init__(self, config: Dict):
        self.FILE_EXTENSION_MAPPING = config.get("file_extension_mapping", self._DEFAULT_EXTENSION_MAPPING)
    ...
    def filter_by_languages(self, checklist: Checklist, languages: List[str]) -> Checklist:
        ...
        for item in checklist.items:
            if 'languages' not in item:
                ...
                if languages_lower & item_languages_lower:  # 交集非空
                    filtered_items.append(item)
...
def calculate_coverage(self, checklist: Checklist, issues: List[Dict[str, Any]]) -> Dict:
    """V8 修复：明确 results 参数类型为 List[Dict]"""
    # 从 issues 中提取已检查的 item ID
...
class ConclusionValidator:
    def validate(
        ...
        # 1. 应用 upgrade_conditions（V8 修复：检查代码内容）
    ...
    def _apply_upgrade_conditions(
        ...
        """V8 修复：根据代码内容（不是 issue 描述）判断是否升级"""
        upgrade_rules = {}
        for item in checklist.items:
    ...
    def _match_condition_in_code(self, condition: str, code: str) -> bool:
        """V8 修复：检查代码内容是否匹配升级条件"""
        if not code:
            return False
        # 提取条件中的关键词（去掉中文描述）
        # 例如："使用 dangerouslySetInnerHTML" -> "dangerouslySetInnerHTML"
        keywords = re.findall(r'[a-zA-Z_][a-zA-Z0-9_.]*', condition)
...
class ChecklistManager:
    ...
    def _downgrade_severity(self, current: str, minimum: str) -> str:
        """降级严重等级"""
        ranks = {"P0": 3, "P1": 2, "P2": 1}
        current_rank = ranks.get(current, 0)
        min_rank = ranks.get(minimum, 0)
        
        if current_rank > min_rank + 1:
            return list(ranks.keys())[list(ranks.values()).index(current_rank - 1)]
        return minimum
```
...
project:
  industries: [healthcare, finance]  # V8：统一不带连字符
  ...
  severity_overrides:
    - industry: finance
      overrides:
        D-07: P0
        S-06: P0
    - industry: healthcare  # V8：与 project.industries 一致
      overrides:
        S-11: P0
        D-16: P0
...
| 问题 | 级别 | 修复内容 |
|------|------|----------|
| L63 正则表达式语法错误 | P0 | `-` 放在字符类末尾 |
| industry 名称不一致 | P0 | 统一为不带连字符 |
| filter_by_languages 大小写敏感 | P1 | 统一转小写 |
| _match_upgrade_condition 子字符串匹配 | P1 | 改为检查代码内容 |
| auto_downgrade_threshold 硬编码 | P1 | 按规则配置 |
| calculate_coverage results 类型未定义 | P1 | 明确为 List[Dict] |
| Token 计算未说明来源 | P2 | 补充分解说明 |
| fallback_checklist 只有 5 项 | P2 | 扩充到 10 项 |
| warnings 输出位置未定义 | P2 | 增加渲染配置 |
