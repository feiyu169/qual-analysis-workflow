...
class TaskClassification:
    ...
    files: List[str]
    lines: int
    skip_heavyskill: bool  # 是否跳过 HeavySkill
```
...
#### 5. Appeal Handler（申诉处理器）⭐ 新增

**职责**：处理用户对 REJECT 结果的申诉。
...
class ParallelReasoning:
    ...
    async def run_async(self, query: str, file_path: str, k: int = 8) -> List[Trajectory]:
        ...
        # 过滤异常结果
        valid_trajectories = [t for t in trajectories if not isinstance(t, Exception)]
        return valid_trajectories
    
    async def _call_llm_async(self, session: aiohttp.ClientSession, 
                              query: str, file_path: str, index: int) -> Trajectory:
        """异步调用 LLM API"""
        # 构建 prompt
        prompt = self._build_prompt(query, file_path, index)
    ...
    def run(self, query: str, file_path: str, k: int = 8) -> List[Trajectory]:
        ...
        内部调用 asyncio.run(self.run_async(...))
        """
...
```python
class ParallelReasoningFallback:
    """伪并行降级方案 - 当 asyncio 不可用时"""
    ...
    def run(self, query: str, file_path: str, k: int = 8) -> List[Trajectory]:
        ...
        trajectories = []
        for i in range(k):
            trajectory = self._call_llm(query, file_path, i)
...
```
V1（当前）：内置静态 YAML 清单
├── 安全审查清单（security.yaml）
├── 架构审查清单（architecture.yaml）
├── 性能审查清单（performance.yaml）
├── API 审查清单（api.yaml）
├── 数据库审查清单（database.yaml）
└── 部署审查清单（deployment.yaml）
...
1. 读取待审查文件
2. 构建 prompt
3. **真正并发**生成 K 条推理轨迹（asyncio + aiohttp）
4. 过滤低质量轨迹
...
| 模式 | K 值 | 单轨迹延迟 | 总延迟（真并行） | 总延迟（伪并行） |
|------|------|-----------|-----------------|-----------------|
| 快速模式 | 4 | 30-60s | 30-60s | 2-4min |
| 标准模式 | 8 | 30-60s | 30-60s | 4-8min |
| 高质量模式 | 16 | 30-60s | 30-60s | 8-16min |
...
# 并行模式配置
parallel_mode: async  # async（真并行）/ fallback（伪并行）
...
name: "安全审查清单"
version: "1.0"
source: builtin  # builtin / learned / custom
...
class Checklist:
    ...
    source: str  # builtin / learned / custom
...
| 阶段 | 最乐观 | 最悲观 | 说明 |
|------|--------|--------|------|
| Phase 0-2 | 20s | 30s | 预处理 |
| Phase 3 Stage 1 | 30s | 60s | 真并行，取决于 LLM 响应 |
| Phase 3 Stage 2 | 1min | 2min | 审议 |
| Phase 3 Stage 3 | 10s | 30s | 校验 |
| Phase 4-5 | 30s | 60s | 门禁+报告 |
| **总计** | **2min** | **5min** | 不含申诉 |

**说明**：
- 使用 asyncio 真并行，Stage 1 延迟 = max(单轨迹延迟)，而非 sum
- 如果 LLM 响应慢（>60s），总延迟可能到 5-7 分钟
- 如果使用伪并行（fallback），总延迟可能到 10-15 分钟
...
1. **快速模式**：K=4，成本减半，效果略降
2. **智能跳过**：小改动（<50行）跳过 HeavySkill
3. **缓存机制**：相同文件的审查结果缓存 1 小时
4. **批量审查**：多个 MR 合并审查
...
hgf:
  task_classifier:
    enabled: true
    default_level: L2
    skip_heavyskill_threshold: 50  # <50 行跳过 HeavySkill
  
  risk_assessor:
    enabled: true
    default_risk: medium
    skip_heavyskill_threshold: low  # 低风险跳过 HeavySkill
...
heavyskill:
  ...
  top_p: 0.95
  timeout: 600
  parallel_mode: async  # async / fallback
...
checklists:
  ...
  domains:
    ...
    - deployment
    - frontend  # 新增
  dynamic_loading: true  # 根据 MR 文件动态加载
...
- [ ] 定义 HGF 组件接口（TaskClassifier、RiskAssessor、GateExecutor、AppealHandler）
- [ ] 定义 HeavySkill 组件接口（ParallelReasoning、SequentialDeliberation、ConclusionValidator）
- [ ] 定义辅助组件接口（QueryEnhancer、ChecklistManager、ReportGenerator）
- [ ] 实现接口适配层
- [ ] 编写接口文档
...
| V3.0 | 2026-06-21 | 根据审查意见修复：延迟预估、成本量化、并行实现、清单来源、申诉机制 |
